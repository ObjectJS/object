...
function HTML5History(history, adapter) {
	...
	if (adapter) {
		...
		if (adapter.blankHTML) self.blankHTML = adapter.blankHTML;
	}
...
HTML5History.prototype = {
	makeIFrame: function(callback) {
		...
		if (document.domain != location.hostname) iframe.setAttribute('src', 'http://' + document.domain + this.blankHTML);
		...
		iframe.attachEvent('onload', function() {
			...
			// 但是！在IE中没有任何缓存的情况下首次访问的话，还是会取不到contentWindow，除了加长setTimeout的时间之外，目前没有好的解决办法
			setTimeout(function() {
				callback();
			}, 0);
		});
