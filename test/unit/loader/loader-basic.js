...
test('buildFileLib', function() {
	...
	ok(pkg.id == 'test_module.js', 'module test_module is added, id is ok');
...
test('add-usage', function() {
	...
	equal(loader.lib['c.js'].dependencies.length, 2, 'c dependencies a and b, so lib[c].dependencies.length = 2');

	loader.add('d/dd', 'a,b,c', function() {});
	ok(loader.getModule('d/dd'), 'd.dd are added to loader.lib');
	equal(loader.lib['d/dd.js'].dependencies.length, 3, 'd.dd dependencies a ,b and c, so lib[d.dd].dependencies.length = 3');
