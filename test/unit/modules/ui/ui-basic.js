...
object.use('ui/ui2.js', function(ui) {
	...
	var TestComponent = new Class(ui.Component, function() {
		...
		this.free2 = ui.define(false, function(self, make) {
			return [make(), document.createElement('div')];
		});
		this.free3 = ui.define(false, function(self, make) {
			make();
			make();
		});
	});
	...
	var test = new TestComponent(document.createElement('div'), {
		...
		'free3.meta.template': '<div>{{value}}</div>'
	});
	...
	test.render('free2', {value: 'free'}, function() {
		...
		equal(test.free2.getNode()[1].innerHTML, '', 'render free components ok.');
	});

	// 没有返回值
	test.render('free3', {value: 'free'}, function() {
		equal(test.free3.getNode().length, 2, 'render free components without return ok.');
		equal(test.free3.getNode()[0].innerHTML, 'free', 'render free components without return ok.');
	});
});
});
