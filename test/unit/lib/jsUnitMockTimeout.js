...
//
// MODIFY:
// 		2011-12-27	support Clock.tick() with no arguments by wangjeaf(zhifu.wang@gmail.com)
...
var Clock = {
    ...
    tick: function(millis) {
		// no millis, means run all functions in array
		if (!millis) {
			this.runFunctionsWithinRange();
		} else {
    ...
    runFunctionsWithinRange: function(oldMillis, nowMillis) {
        var scheduledFunc;
        var funcsToRun = [];
		// if no oldMillis or now Millis, means run all functions
		oldMillis = oldMillis || -1;
		nowMillis = nowMillis || 10000000;
