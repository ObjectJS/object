...
var Element = this.Element = new Class(/**@lends dom.Element*/ function() {
	...
	this.initialize = function(self, tagName) {
		...
		if (self.get('draggable') == true 
			&& (self.tagName != 'IMG' && self.tagName != 'A')) {// && !_supportHTML5Drag) {
			if(self._canDrag != true) {
				self.makeDraggable();
			}
		}
		if (self.get('dropzone') != undefined && self.get('dropzone') != "") { // && !_supportHTML5Drop) {
			//dropzone需要触发dragenter dragover dragleave drop
			if(self._canDrop != true) {
				self.makeDroppable();
			}
		}
	};
	...
	this.dropzone = property(
		...
		function(self, dropzone){
			...
			if(dropzone != undefined && dropzone != '') {
				...
				self.makeUndroppable();
			}
		}
	);
	/**
	 * 让当前实例具备拖拽功能
	 * 元素本身触发的事件包括：dragstart drag dragend
	 * 容器触发的事件包括：dropinit, dragenter, dragover, dragleave, drop
	 */
	this.makeDraggable = function(self) {
		if(self._canDrag != true) {
			...
			while(parent && parent.tagName != 'BODY' && parent.tagName != 'HTML') {
				if(parent.dropzone != undefined && parent.dropzone != '') {
					parent = wrap(parent);
					self._belongTo = parent;
					self._droppables.push(parent);
					break;
				}
	...
	this.makeUndraggable = function(self) {
		if(self._canDrag == true) {
			self.removeEvent('mousedown', self.__handleMouseDown, false);
			self._belongTo = null;
			self._droppables = null;
			self._canDrag = false;
		}
	}

	this.makeDroppable = function(self) {
		if(self._canDrop != true) {
			self._canDrop = true;
		}
	}
	this.makeUndroppable = function(self) {
		if(self._canDrop == true) {
			self._canDrop = false;
		}
	}
	...
	this.addDroppable = function(self, droppable, isInit) {
		...
		self._droppables = self._droppables || [];
		self._droppables.push(droppable);
		if(isInit) {
			self._belongTo = droppable;
		}
	}
	...
	this.__handleMouseDown = function(self, e) {	
		...
		self._originX = pos.x;
		self._originY = pos.y;
		self._deltaX = pos.x - pos2.x;
		self._deltaY = pos.y - pos2.y;
		//self.fireEvent('draginit', {ele:self, event:e});
		var doc = wrap(document);
		//给document的mousemove 和 mouseup加上事件
		doc.addEvent('mousemove', checkDragging, false);
		doc.addEvent('mouseup', cancel, false);
		if(ua.ua.ie) {
			doc.addEvent('selectstart', eventStop, false); 
		} else {
			doc.addEvent('mousedown', eventStop, false);
		}

		function checkDragging(e) {
			var pos = getMousePos(e);
			var distance = Math.round(Math.sqrt(Math.pow(pos.x - self._originX, 2) + 
					Math.pow(pos.y - self._originY, 2)));
			if(distance > 2) {
				doc.removeEvent('mousemove', checkDragging, false);
				...
				if(self._belongTo) {
					self._belongTo.fireEvent('dropinit', {dragging:self, event:e});
				}
			}
		}
		function dragging(e) { 
			if(e.preventDefault) e.preventDefault();
			var pos = getMousePos(e);
			self.style.left = (pos.x - self._deltaX) + 'px';
			self.style.top  = (pos.y - self._deltaY) + 'px';
			...
			for(var i=0,current,currentPos,l=self._droppables.length; i<l; i++) {
				current = self._droppables[i];
				...
				if(current == self._belongTo) {
					...
					if(isInContainer(containerCoordinates, draggingCoordinates)) {
						if(self._belongTo) {
							self._belongTo.fireEvent('dragleave', {from:self._belongTo, to:current, dragging:self});
						}
						current.fireEvent('dragenter', {from:self._belongTo, to:current, dragging:self});
						self._belongTo = current;
					}
				}
			}
		}
		...
		function stop(e) {
			...
			if(ua.ua.ie) {
				doc.removeEvent('selectstart', eventStop, false); 
			} else {
	...
	this.addDraggables = function(self, draggables, isInit) {
		...
		for(var i=0; i<draggables.length; i++) {
			...
			self._draggables.push(draggables[i]);
